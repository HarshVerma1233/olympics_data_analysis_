...
def medal_tally(df):
    # Drop duplicates for accurate medal counts
    medal_tally = df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'])
    
    # Group by 'region_name'
    medal_tally = medal_tally.groupby('region_name').sum()[['Gold', 'Silver', 'Bronze']].sort_values('Gold', ascending=False).reset_index()
...
def most_successful(df, sport):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(15).merge(df, left_on='Name', right_on='Name', how='left')[
        ['Name', 'count', 'Sport', 'region_name']].drop_duplicates('Name')
    return x 

def yearwise_medal_tally(df, country):
    temp_df = df.dropna(subset="Medal")  # Medal has Nan values, so removed it
